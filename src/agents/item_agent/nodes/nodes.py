...
def get_item_details_node(offer_description : str = None) -> ItemDetails:
    generator_model = model.with_structured_output(ItemDetails)
    generator = get_item_details_prompt | generator_model
    flow_json = read_json(JSON_PATH)
    option_paths = get_option_paths(flow_json)
    category_list = get_propmt_list(option_paths)
    ...
    if isinstance(response, ItemDetails):
        return response
    else:
        return None
...
if __name__ == "__main__":
    ...
    print(get_item_details_node(item_description=description))
